...
async function sendOrderConfirmationEmail({ email, firstName, order, items, bank }) {
  ...
  const content = `
    ...
    <div style="background:#111113;border:1px solid rgba(19,40,83,0.4);border-radius:10px;padding:22px 24px;margin-bottom:28px;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        ${bank.beneficiary ? `<tr><td style="font-size:12px;color:rgba(255,255,255,0.35);padding:4px 0;width:130px;">Titulaire</td><td style="font-size:14px;color:#fff;font-weight:600;padding:4px 0;">${bank.beneficiary}</td></tr>` : ''}
        ${bank.iban ? `<tr><td style="font-size:12px;color:rgba(255,255,255,0.35);padding:4px 0;width:130px;">IBAN</td><td style="font-size:15px;color:#fff;font-weight:800;letter-spacing:0.04em;padding:4px 0;font-family:monospace;">${bank.iban}</td></tr>` : ''}
        ${bank.bic ? `<tr><td style="font-size:12px;color:rgba(255,255,255,0.35);padding:4px 0;width:130px;">BIC</td><td style="font-size:14px;color:#fff;font-weight:600;padding:4px 0;font-family:monospace;">${bank.bic}</td></tr>` : ''}
        <tr><td style="font-size:12px;color:rgba(255,255,255,0.35);padding:4px 0;width:130px;">Type</td><td style="font-size:14px;color:rgba(255,255,255,0.65);padding:4px 0;">${bank.transferType || 'SEPA'}</td></tr>
        ${order.paymentReference ? `<tr><td style="font-size:12px;color:rgba(255,255,255,0.35);padding:4px 0;width:130px;">Motif</td><td style="font-size:15px;color:#132853;font-weight:800;letter-spacing:0.03em;padding:4px 0;font-family:monospace;">${order.paymentReference}</td></tr>` : ''}
      </table>
      <div style="margin-top:12px;font-size:12px;color:rgba(255,255,255,0.3);line-height:1.6;">Merci d'indiquer le motif lors de votre virement afin que nous puissions associer le paiement à votre commande.</div>
    </div>` : ''}
...
async function sendLeadEmail({ type = 'contact', data = {} }) {
  ...
  const rows = Object.entries(data)
    ...
    .map(([k, v]) => `
      <tr>
        <td style="font-size:11px;font-weight:800;letter-spacing:0.12em;text-transform:uppercase;color:rgba(255,255,255,0.35);padding:7px 0;width:150px;vertical-align:top;">${k}</td>
        <td style="font-size:14px;color:#fff;padding:7px 0;word-break:break-word;">${String(v).replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br/>')}</td>
      </tr>`).join('');

  const content = `
    <div style="text-align:center;margin-bottom:28px;">
      <div style="width:64px;height:64px;background:rgba(19,40,83,0.1);border:2px solid rgba(19,40,83,0.35);border-radius:50%;display:inline-block;line-height:64px;font-size:28px;margin-bottom:16px;">${label.icon}</div>
      <h1 style="margin:0 0 8px;font-size:26px;font-weight:900;color:#ffffff;letter-spacing:-0.02em;">${label.title}</h1>
      <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.45);">Reçu via ${getPublicSiteUrl()}</p>
    </div>
    <table width="100%" cellpadding="0" cellspacing="0">
      ${rows}
    </table>
    <div style="text-align:center;margin-top:28px;">
      <a href="${getPublicSiteUrl()}" style="display:inline-block;background:#132853;color:#fff;text-decoration:none;font-size:13px;font-weight:800;letter-spacing:0.1em;text-transform:uppercase;padding:14px 28px;border-radius:6px;">
        ${label.reply}
      </a>
    </div>
  `;
